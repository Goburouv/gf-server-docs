...
| 9 | `m_eEnchantCategory` | uint (V.10) → `Category` (`GameData::EEnchantCategory`, `SEffectData+0x18`) | **No reader located** (128 rows use it). Searched: `SEffectData` exposes only 5 methods and none touches `+0x18`; `CEnchantMgr::Add @0xa1ef40`, `ILifeEntity::AddEnchant @0x95a6e0`, `CEffectFactory::EffectToLife @0x4489a0` and `CheckEffectToLife @0x449320` decompiled in full without reading it; the 41 `CCheck*` predicates read `+0x00` (`ID`) or `+0x10` (`Type`), never `+0x18`; and the 8 two-instruction sequences that use the real bases of a `SEffectData` (`[CEnchant+0x10]`, `[CEffectCommand+0x8]`, `[ISpell+0x18]`) return 0 hits. Not a proof of absolute absence. A second pass on 2026-09-04 cleared 14 more functions with the same result: both `CEnchant` constructors (`0xa1a940`, `0xa1a9d0`), `CEnchantMgr::Reset @0xa1fd40`, `Load @0xa205f0`, `AddToContainers @0xa26270`, `HasAnyDebuff @0xa22720`, `CEnchant::Reg2Caster @0xa1af50`, `ISpell::OnHitTargetEffect @0x9b3de0`, `UpdateOnHitTargetEffect @0x9b2680`, `CEffectFactory::ParseEffectCommands @0x449e00`, `SEffectData::GetDataString @0xa18dd0` (the GM dump behind `TC_GetEffectData @0x7d5fa0` — it prints only `ID`) and `UseItemControl::CheckExecute @0x9aa450`. `AddToContainers`, which files a new enchant into some twenty dispatch containers, reads `ID`, `EnchantType` (`+0x2fa`), `WeaponFlags`, `HurtType` and eighteen vector-empty checks, and never `+0x18`. **The exhaustive machine sweep remains undone, and cannot be done with this tooling:** `insn_query`'s operand matcher inspects only immediate and register operands, never the displacement of a memory operand — querying `op_any=24` against the known writer `mov [r13+18h], ecx @0x782f64` returns zero hits — and the text fallback produces upwards of 13,000 raw `+18h` matches across `.text`, almost all libc. So the verdict stays *no reader located* and must not be upgraded to *no reader*. |
| 10 | `m_nImmuneMonsterType` | ushort → `ImmuneMonsterType` (`SEffectData+0x1c`, parent and child) | A **bitmask of immune creature categories**, not a type id. `ISpell::OnHitTargetEffect @0x9b3de0` (site `0x9b3e72`) and `ISpell::OnHitLife` (`0x9b30fd`, `0x9b3381`) call the victim's `vtable+0xA0` virtual, run `test ImmuneMonsterType, category` and, if the AND is ≠ 0, **do not apply the effect**. The overrides of that virtual are `CMonster::GetCategory @0x8cedb0` (reads the mob template at `+0x158`) and `CCharacter::GetCategory @0x947d00`, which returns a **constant 4**: setting bit `4` makes the effect harmless against players — the 7 rows holding `126` (= 2+4+8+16+32+64) carry it. The identity of the virtual is **proven by slot index**, not inferred: the Itanium ABI puts the vptr at `_ZTV+0x10`, so slot `vptr+0xA0` is the qword at `_ZTV+0xB0` — `_ZTVN5lapis8CMonsterE @0xdc7308` gives `0x8cedb0` (`CMonster::GetCategory`) and `_ZTVN5lapis10CCharacterE @0xdc0c38` gives `0x947d00` (`CCharacter::GetCategory`), while `_ZTVN5lapis11ILifeEntityE @0xdc6058` holds the pure-virtual thunk `0xbb26a0`. The call site `mov rax,[victim]; call qword ptr [rax+0A0h] @0x9b3e6c` is followed by `movzx ecx, word ptr [effect_data+1Ch]; cwde; test ecx,eax`, which also confirms `ImmuneMonsterType` is read as a **word** at `SEffectData+0x1c`. |
...
|---|---|---|
| 0x1 / 0x2 / 0x4 | 326 / 429 / 108 | The buff ends when the carrier attacks / is attacked / takes damage. `CSpellDBO::LoadAllEffectData` injects the text command `Finish 100` into `CommandsAttack` (`+0x88`), `CommandsBeAttacked` (`+0xd0`) or `CommandsHurt` (`+0xe8`); gates at `0x784c44` / `0x784cfa` / `0x784db1` for the parent and `0x782f68` / `0x78300c` / `0x7830b3` for the transition child. |
...
| 0x2000 / 0x20000 | 18 / 54 | Ends on item use / on move: same `Finish 100` injection into `CommandsUseItem` (`+0x298`) / `CommandsMoveSelf` (`+0x2b0`); gates at `0x784e68` / `0x784f1f` (parent) and `0x78315a` / `0x783201` (child). Those two vectors have **no other source**, so without these bits they stay empty. |
...
| 0x10000 | 6 | **No reader located.** The other four bits of the same byte (`Flag+0x16`) do show up and are all identified, so the sweep is reliable: this bit is never tested. Strengthened on 2026-09-04 by two further facts. First, `EnchantFlag` never leaves `SEffectData`: both `CEnchant` constructors (`@0xa1a940`, `@0xa1a9d0`) set `CEnchant::m_Flags` (`+0x44`) to `0` unconditionally, so that field is a **disjoint runtime bitfield, not a copy of the data flag** — the bit is reachable only through `CEnchant+0x10 → Flag`, `CEffectCommand+0x8 → Flag` or `ISpell+0x18`/`+0x20 → Flag`. Second, a dword-level sweep of `test` and `or` against the immediate `0x10000` over the whole of `.text` (`0x400600`–`0xcde8a8`) returns 8 hits and every one is a different bitfield: `CEnchant::Reg2Caster @0xa1af50` tests `m_HurtType` (`CEnchant+0x40`), `UseItemControl::CheckExecute @0x9aa450` tests the item's `m_nOpFlags`, `ILifeEntity::CanDo @0x95b4e0` tests `CLifeEntityFlags` bit 16 (ZoneOut), and the remainder are libstdc++ guard variables and statically linked OpenSSL. There is no `or` writer of the bit either. |
